...
static struct pnlfs_superblock *write_superblock(int fd, struct stat *fstats)
{
	...
	printf("la position superblockwrite %ld\n",lseek(fd, 0, SEEK_CUR));
...
static int write_ifree_blocks(int fd, struct pnlfs_superblock *sb)
{
	...
	int k;
				for(k=0;k<64;k++)
					printf("%d",IS_SET(ifree[0],63-k));
				printf("\n");
	printf("res %ld %ld\n",EXTRACT_BIT(ifree[0],0),EXTRACT_BIT(ifree[0],1));
...
static int write_data_blocks(int fd, struct pnlfs_superblock *sb)
{
	...
	/* Root block (/) */
	int test = lseek(fd, 0, SEEK_CUR);
	printf("la position datawrite %d\n",test);
	printf("test / 4096 : %d\n",test /4096);
